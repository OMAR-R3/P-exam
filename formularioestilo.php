    ...
    <section id="global">
        ...
        <header>
            <div id="logo" style="width: 100%; height: 100%;">
                <h1>Examen de Matematicas - Multiples</h1>
            </div>
...
$registros = mysqli_query($conexion, "SELECT nombreAlumno,grado,grupo,turno,correo from loginalumnoa where usuarioalumno='$_REQUEST[login]' AND contraalumno='$_REQUEST[password]'") or die("Problemas en el select:" . mysqli_error($conexion));

while($lib = mysqli_fetch_array($registros)){

    $nombre = $lib['nombreAlumno'];
    $grado = $lib['grado'];
    $grupo = $lib['grupo'];
    $turno = $lib['turno'];
    $correo = $lib['correo'];
    ...
    $registros = mysqli_query($conexion, "SELECT nombreAlumno,grado,grupo,turno,correo from loginalumnoa where usuarioalumno='$usuario'") or die("Problemas en el select:" . mysqli_error($conexion));

    if($lib = mysqli_fetch_array($registros)){
    $nombre= $lib['nombreAlumno'];
    $grado= $lib['grado'];
    $grupo= $lib['grupo'];
    $turno= $lib['turno'];
...
?>
        ...
        <form action="salidaestilo.php" method="post" style="font-size: 18px;">

            
            <div class="cajita1" style="width: 100%; margin: 1px 0; padding: 3px 0; background:rgb(247, 250, 166); text-align:center;">



                <label>Nombre:</label>
                <input type="text" name="nombre" required style="width: 300px; border: 2px solid rgba(0, 0, 0, 0);" placeholder="Ingresa tu nombre:" value="<?php echo$nombre;?>">
                <br>
                <label>Grado:</label>
                <input type="text" name="grado" maxlength="1" required style="width: 300px; border: 2px solid rgba(0, 0, 0, 0);" placeholder="Ingresa tu grado escolar:" value="<?php echo$grado;?>">
                
                

                <br>
                <label>Grupo:</label>
                <input type="text" name="grupo" maxlength="1" required style="width: 300px; border: 2px solid rgba(0, 0, 0, 0);" placeholder="Ingresa tu grupo:" value="<?php echo$grupo;?>">
                <br>
                <label>Turno:</label>
                <input type="text" name="turno" maxlength="1" required style="width: 300px; border: 2px solid rgba(0, 0, 0, 0);" value="<?php echo$turno;?>">
                <br>
                <label>Correo:</label>
                <input type="email" name="correo" required style="width: 300px; border: 2px solid rgba(0, 0, 0, 0);" placeholder="Ingresa tu correo (opcional):"  value="<?php echo$correo;?>">
            </div>
            <br>
            ...
            <div class="cajita" style="width: 100%; text-align:center; "><input type="submit" name="Confirmar" value="VERIFICAR RESPUESTAS" style="border-radius: 10px; background: rgb(48, 170, 207); color: white;"></div>
        </form>
        <?php
    }
